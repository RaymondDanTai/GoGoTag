...
/* 載入頁面擁有者資訊 */
function loadPersonalPageInfo() {
	var currentURL = window.location.href;
	var currentFolder = currentURL.substring(0, currentURL.lastIndexOf('/'));
	var thePageOwnerSession = currentFolder.substring(currentFolder.lastIndexOf('/') + 1);
	var userId = localStorage.getItem("userId");
	var formData = new FormData();
	formData.append("thePageOwnerSession", thePageOwnerSession);
	formData.append("userId", userId);
	var xhr;
	if (window.ActiveXObject) {
		xhr = new ActiveXObject("Microsoft.XMLHTTP");
	...
	xhr.open("POST", "/SocialMedia/GetPageOwnerInformation", true)
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				var jsonObj = JSON.parse(xhr.responseText);
				var pageOwnerAvatarURL = jsonObj.pageOwnerAvatarURL;
				var pageOwnerName = jsonObj.pageOwnerName;
				var isPageowner = jsonObj.isPageowner;
				var friendshipStatusCode = jsonObj.friendshipStatusCode;
				document.getElementById("left-user-avatar").setAttribute("src", pageOwnerAvatarURL);
				document.getElementById("left-user-name").append(pageOwnerName);
				if (!isPageowner) {
					var setFriendshipButton = document.createElement("button");
					setFriendshipButton.setAttribute("type", "button");
					setFriendshipButton.setAttribute("id", "friendship-button");
					setFriendshipButton.setAttribute("onclick", "checkFriendshipStatus()")
					if (friendshipStatusCode == 0 || friendshipStatusCode == -1) {
						setFriendshipButton.append("添加好友");
					} else if (friendshipStatusCode == 1) {
						setFriendshipButton.append("刪除好友");
					} else if (friendshipStatusCode == 2) {
						setFriendshipButton.append("等待確認");
					}
					document.getElementById("friendship-area").append(setFriendshipButton);
					var isOwner = false;
					showPageOwnerPosts();
				} else {
					var isOwner = true;
...
function showPageOwnerPosts() {
	...
	xhr.open("POST", "/SocialMedia/GetPageOwnerPosts", true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			for (var i = 0; i < jsonArray.length; i++) {
				...
				setPostImg.setAttribute("src", postImgURL); //變數
...
function createPostDeleteSection(event) {
	...
	setPostDeleteConfirmText.append("是否確認刪除留言？")
...
function checkFriendshipStatus() {
	...
	var currentURL = window.location.href;
	var currentFolder = currentURL.substring(0, currentURL.lastIndexOf('/'));
	var pageOwnerSession = currentFolder.substring(currentFolder.lastIndexOf('/') + 1);
	var formData = new FormData();
	formData.append("currentUserId", currentUserId);
	formData.append("pageOwnerSession", pageOwnerSession);
	var xhr;
	if (window.ActiveXObject) {
		xhr = new ActiveXObject("Microsoft.XMLHTTP");
	} else if (window.XMLHttpRequest) {
	...
	xhr.open("POST", "/SocialMedia/CheckFriendshipStatus", true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				var jsonObj = JSON.parse(xhr.responseText);
				var friendshipStatusCode = jsonObj.friendshipStatusCode;
				if (friendshipStatusCode == 0 || friendshipStatusCode == -1) {
					inviteFriend(currentUserId, pageOwnerSession);
				} else {
					removeFriend(currentUserId, pageOwnerSession);
				}
			}
		}
	};
...
/* 刪除好友 */
function removeFriend(currentUserId, pageOwnerSession) {
	var formData = new FormData();
	formData.append("currentUserId", currentUserId);
	formData.append("pageOwnerSession", pageOwnerSession);
	var xhr;
	...
	xhr.open("POST", "/SocialMedia/RemoveFriend", true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				var jsonObj = JSON.parse(xhr.responseText);
				var isFriendRemoved = jsonObj.isFriendRemoved;
				if (isFriendRemoved == true) {
					document.getElementById("friendship-button").innerHTML = "添加好友";
					var element = document.getElementById("friendList-contains");
					if (element) {
						while (element.firstChild) {
							element.firstChild.remove();
						}
...
/* 添加好友 */
function inviteFriend(currentUserId, pageOwnerSession) {
	...
	xhr.open("POST", "/SocialMedia/InviteFriend", true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				var jsonObj = JSON.parse(xhr.responseText);
				var isFriendInvited = jsonObj.isFriendInvited;
				if (isFriendInvited) {
					document.getElementById("friendship-button").innerHTML = "刪除好友";
					var element = document.getElementById("friendList-contains");
					if (element) {
						while (element.firstChild) {
							element.firstChild.remove();
...
function getFriendAvatar(currentUserId, pageOwnerSession) {
	var maintenanceAction = "getFriendAvatar";
	var formData = new FormData();
	...
	xhr.open("POST", "/SocialMedia/FriendshipMaintenance", true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				var jsonObj = JSON.parse(xhr.responseText);
