...
public class ShowAllPosts extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		if(sqlSelect.equals("0")) {
			sql = "SELECT post_id, post_text, post_img, updated_at, like_count, tags_id, poster_id, location_id FROM posts WHERE tags_id IN (1, 2, 3, 4) AND updated_at <= ? ORDER BY updated_at DESC LIMIT 5";
		} else {
			sql = "SELECT post_id, post_text, post_img, updated_at, like_count, tags_id, poster_id, location_id FROM posts WHERE tags_id IN (1, 2, 3, 4) AND updated_at < ? ORDER BY updated_at DESC LIMIT 5";
		}
